...
int brute(vector<int>& pv, const int loc, const int row, const int bw, const int mxR) {
    ...
    if (loc >= (int)pv.size() - 1) {
        ans[row] = loc;
        return 0;
    }
    if (dp.count(row) && dp[row].count(loc)) return dp[row][loc];
    for (; loc2 < (int)pv.size() && pv[loc2] - (loc == 0 ? 0 : pv[loc - 1]) <= bw; loc2++) {
        if (loc2 == pv.size() - 1) {
            mn_C_Loc = { 0,loc2 };
            break;
        }
        t = (int)pow(bw - (pv[loc2] - (loc == 0 ? 0 : pv[loc - 1])), 3) + brute(pv, loc2 + 1, row + 1, bw, mxR);
...
int get_mxR(vector<int>& v, const int bw) {
    int r = 0;
    for (int i = 0, s = 0; i < (int)v.size(); i++) {
        s += v[i];
        if (s > bw) { r++; s = v[i]; }
    }
    return r;
...
vector<int> prefix(vector<int>& v) {
    vector<int> pv = v;
    for (int i = 1; i < (int)v.size(); i++) pv[i] += pv[i - 1];
    return pv;
}
